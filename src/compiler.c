...
static void ifStatement() {
  ...
  int thenJump =
      emitJump(OP_JUMP_IF_FALSE); // if false it will jump to else block
  emitByte(OP_POP);
  statement();
  int elseJump =
      emitJump(OP_JUMP); // jump over else block to end unconditionally
  patchJump(thenJump);
  emitByte(OP_POP);
  if (match(TOKEN_ELSE))
    statement();
...
static void forStatement() {
  ...
  if (!check(TOKEN_SEMICOLON)) {
    expression();
    consume(TOKEN_SEMICOLON, "Expect ';' to separate condition in for loop.");
...
static void call(bool _canAssign) {
